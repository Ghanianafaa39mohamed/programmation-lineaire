...
static const double epsilon   = 1.0e-8; // calcul precision
...
void simplex(Tableau *tab) {
  ...
  while( ++loop ) {
    ...
    if( pivot_col < 0 ) {
      printf("Valeur optimale en A[0,0]=%3.2lf (pas de valeurs négatives pour la ligne en 0).\n",
        tab->mat[0][0]);
